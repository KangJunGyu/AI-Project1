...
class tour:
    ...
    def bindClusters(self):
        ...
        for i in range(self.cluster_number):
            ...
            temp_array.append(self.cluster_array[visited[i]])
            print(array[i])
        ...
        for node in range(len(cluster_array)): 
            if node not in visited:
                ...
                if depth == 0 and  node != 0:
                    ...
                    total_length = total_length + cluster_array[visited[depth-1]].connectClusters(cluster_array[visited[depth]])
                visited[depth] = node
...
class cluster:
    ...
    def inputCity(self, city):
        ...
        self.length = self.length + self.cities_order[len(self.cities_order)-2].getDistance(self.cities_order[len(self.cities_order)-1])
        self.defineEndNode()
